...
The solution uses Github actions to deploy. When a new commit is merged to main, the [CI/CD workflow]() will deploy it to the internal test environment (https://altinn-dev-api.azure-api.net) where the developer can test it. They then have to go back to the workflow and approve it for further deployment to staging and production.
