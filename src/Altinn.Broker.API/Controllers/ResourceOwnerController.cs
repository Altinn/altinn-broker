...
public class ResourceOwnerController : Controller
{
    ...
    [HttpPost]
    public async Task<ActionResult> InitializeResourceOwner([FromBody] ResourceOwnerInitializeExt resourceOwnerInitializeExt, [ModelBinder(typeof(MaskinportenModelBinder))] CallerIdentity token, CancellationToken ct)
    {
        ...
        BackgroundJob.Enqueue(
            () => _resourceManager.Deploy(resourceOwner!, ct)
        );
    ...
    public async Task<ActionResult<ResourceOwnerOverviewExt>> GetResourceOwner(string resourceOwnerId, CancellationToken ct)
    {
        ...
        var deploymentStatus = await _resourceManager.GetDeploymentStatus(resourceOwner, ct);
