...
public class LegacyGetFilesQueryHandler : IHandler<LegacyGetFilesQueryRequest, List<Guid>>
{
    ...
    private async Task<List<ActorEntity>> GetActors(string[] recipients, CancellationToken ct)
    {
        List<ActorEntity> actors = new();
        foreach (string recipient in recipients)
        {
            ActorEntity entity = await _actorRepository.GetActorAsync(recipient, ct);
            actors.Add(entity);
        }
    ...
    public async Task<OneOf<List<Guid>, Error>> Process(LegacyGetFilesQueryRequest request, CancellationToken ct)
    {
        ...
        {
            fileSearch.Actors = await GetActors(request.Recipients, ct);
        }
        else
        {
            fileSearch.Actor = await _actorRepository.GetActorAsync(request.OnBehalfOfConsumer ?? string.Empty, ct);
        ...
        return await _fileRepository.LegacyGetFilesForRecipientsWithRecipientStatus(fileSearch, ct);
    }
}
