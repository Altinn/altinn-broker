...
In Azure, Front Door can do the same: set GitHub secret `API_ORIGIN_HOST_NAME` to the APIM host
(e.g. `altinn-dev-api.azure-api.net`). Then `/broker/*` is forwarded to APIM and the SPA stays on
`/*`. Leave `VITE_API_BASE_URL` empty so the SPA uses same-origin `/broker/...` URLs (cookies work).

Register this ID-Porten redirect URI (Front Door + API path):

`https://<front-door-host>/broker/api/v1/authentication/callback`
